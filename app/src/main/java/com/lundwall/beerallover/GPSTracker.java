...
public class GPSTracker extends Service implements LocationListener {
   ...
   /*
    ...
    With the parameter type=xml this service returns an xml document with iso country code and country name. The optional parameter lang can be used to specify the language the country name should be in. JSON output is produced with type=JSON
        ...
        url  = gps_site;
        url += "?lat=" +Double.toString(latitude);
        url += "&lng=" +Double.toString(longitude);
        url += "&username="+ gps_user;
        try{
            landekode=asynkTråd.execute("url", url).get();
        } catch (Exception e) {
            Toast.makeText(this.mContext, "Fejl i GPSTracker: "+e.getMessage(), Toast.LENGTH_LONG).show();
        }
        return landekode;
    };
    */
